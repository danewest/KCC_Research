...
CRITICAL_VARS = ['TAIR', 'PRES', 'SM02', 'SM04', 'ST02', 'ST04', 'VT90', 'VT20']
...
def process_site_data(file_path, site_name):
    print(f"\n--- Processing {site_name} Data ---")
    ...
    try:
        ...
        # Ensure critical variables are numeric
        for col in CRITICAL_VARS:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
            else:
                print(f"Warning: Column '{col}' not found in {site_name} data.")

        # Removed redundant SM04 numeric conversion here, as CRITICAL_VARS loop handles it
        # (Based on our earlier discussion about redundancy when SM04 is in CRITICAL_VARS)

        print(f"Successfully loaded {site_name} data. Initial shape: {df.shape}")
        print(f"Initial data types for {site_name}:\\n{df[CRITICAL_VARS].dtypes}")
        print(f"Initial missing values (NaN) in critical vars for {site_name}:\\n{df[CRITICAL_VARS].isnull().sum()}")

    except Exception as e:
        print(f"Error loading {site_name} data: {e}")
        return None

    # 2. Find the date where critical variables all start to have non-empty values
    start_dates = {}
    ...
    df_trimmed = None
    if start_dates and len(start_dates) == len(CRITICAL_VARS):
        common_start_date = max(start_dates.values())
        print(f"Common start date for all critical variables ({', '.join(CRITICAL_VARS)}) in {site_name}: {common_start_date}")
        ...
        print(f"Missing values after trimming in critical vars for {site_name}:\\n{df_trimmed[CRITICAL_VARS].isnull().sum()}")

        # --- Save the trimmed DataFrame to a new CSV ---
        trimmed_output_path = f"{site_name}_trimmed_data.csv"
        df_trimmed.to_csv(trimmed_output_path, index=True) # index=True saves the datetime index as a column
        print(f"Trimmed data saved to: {trimmed_output_path}")

        # --- Identify and record missing values specifically in the TRIMMED data ---
        missing_details = []
        ...
        missing_datetimes_in_trimmed = df_trimmed.index[df_trimmed[CRITICAL_VARS].isnull().any(axis=1)].tolist()
        
        print(f"**Analyzing missing datetimes for {site_name} from {len(missing_datetimes_in_trimmed)} potential points in TRIMMED data.**") # Added verification print
        if missing_datetimes_in_trimmed:
            print(f"**First 5 missing datetimes found in TRIMMED data for {site_name}: {missing_datetimes_in_trimmed[:5]}**") # Added verification print
        ...
        missing_details_output_path = f"{site_name}_missing_details_trimmed.csv"
        if not df_missing_details.empty:
            df_missing_details.to_csv(missing_details_output_path, index=False)
            print(f"Detailed missing value information for trimmed data saved to: {missing_details_output_path}")
            ...
            print(f"No specific missing values found in critical variables for the trimmed data in {site_name}. No '{missing_details_output_path}' file generated.")

    else:
        print(f"Skipping trimming for {site_name} as common start date could not be determined for all critical variables.")
...
# --- Main execution for both sites ---
grdr_df_trimmed = process_site_data('GRDR.csv', 'GRDR')
wood_df_trimmed = process_site_data('WOOD.csv', 'WOOD')

# --- Define variables for gap analysis based on frequency ---
STANDARD_FREQ_VARS = ['TAIR', 'PRES', 'VT90', 'VT20']
SOIL_FREQ_VARS = ['SM02', 'SM04', 'ST02', 'ST04'] # Note: ensure these are in your CRITICAL_VARS list if you want them in the combined list later

# --- Perform gap analysis for GRDR ---
if grdr_df_trimmed is not None:
    # Analyze standard frequency variables (5 minutes)
    analyze_data_gaps(grdr_df_trimmed, STANDARD_FREQ_VARS, 5, 'GRDR', '_standard_gaps')
    # Analyze soil frequency variables (30 minutes)
    analyze_data_gaps(grdr_df_trimmed, SOIL_FREQ_VARS, 30, 'GRDR', '_soil_gaps')

# --- Perform gap analysis for WOOD ---
if wood_df_trimmed is not None:
    # Analyze standard frequency variables (5 minutes)
    analyze_data_gaps(wood_df_trimmed, STANDARD_FREQ_VARS, 5, 'WOOD', '_standard_gaps')
    # Analyze soil frequency variables (30 minutes)
    analyze_data_gaps(wood_df_trimmed, SOIL_FREQ_VARS, 30, 'WOOD', '_soil_gaps')


# --- Notes Document Content Guidance ---
print("\n--- Content for your Notes Document ---")
# ... (rest of the guidance remains the same, including gap filling methods and feature engineering) ...